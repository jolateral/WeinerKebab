    ...
    private void GenerateFloor()
{
    ...
    if (Random.value < fanChance)
    {
        ...
        if (fx != float.MinValue)
            spawnedObjects.Add(Instantiate(fanPrefab,
                new Vector3(fx, obstacleY, 0), Quaternion.identity));
    }

    if (Random.value < wireChance)
    {
        float wx = GetSafeObstacleX(platformSegments);
        if (wx != float.MinValue)
            spawnedObjects.Add(Instantiate(wirePrefab,
                new Vector3(wx, obstacleY, 0), Quaternion.identity));
    }
